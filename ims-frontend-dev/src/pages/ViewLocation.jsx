...
const ViewLocation = () => {
  ...
  const updateLocaitonStock = (quantity, currentQty) => {
    if (!location || typeof location.available === 'undefined' || typeof location.stock === 'undefined') {
...
if(numericCurrentQty !== numericQuantity){
  ...
  const handleChange = (e) => {
    ...
    if (name === "product") {
      setData({
        ...
        "location": "Row: " + location?.row + " Col: " + location?.col + " " + location?.warehouseName,
        "warehouse": {id: location.warehouseId}
      });
    }
    setData((prevData) => ({
  ...
  const handleClick = () => {
    ...
    if (showAdd) {
      const hasNull = Object.entries(data).filter(([key]) => key !== 'id'&& key !== 'warehouse').some(([, value]) => value.length < 1);
      if (hasNull) {
    ...
    <div>
      <div className='m-5'>
        ...
        <table className="table table-collapse table-hover table-bordered">
          <thead>
            {/* <tr key={"header"}>
              {Object.keys(productLocations[0]).map((key) => (
                <th>{key}</th>
              ))}
              <th>Action</th>
            </tr> */}
            ...
            {productLocations?.map((item, index) => (
              <tr key={item?.id} >
                ...
                <td style={{ maxWidth: "100px" }}>
                  {editingRow === index ? (
                    <select style={{ marginTop: "7px", maxWidth: "100px" }}
                ...
                <td style={{ maxWidth: "100px" }}>
                  {editingRow === index ? (
                    ...
                    >
                      ...
                      {pos?.map((item, key) => (
                        <option key={key} value={item?.po}>{item.po}</option>
                      ))}
                    </select>
                  ) : (
                    item?.po
          ...
          <div className="d-flex justify-content-between">
            <form onSubmit={""} className="input-group d-flex flex-row">
              <div className="d-flex flex-column fw-bold">
                ...
                <select style={{ maxWidth: "100px", height: "35px" }}
              ...
              <div className="d-flex flex-column fw-bold " style={{ marginLeft: "5px", width: "130px" }} >
              ...
              <div className="d-flex flex-column fw-bold " style={{ marginLeft: "5px" }}>
        ...
        >
          <div className="mb-2" style={{ width: "100%" }}>
            <button
              ...
              className={`${showAdd ? 'bg-success' : 'bg-secondary'} text-white mt-3`}
              onClick={handleClick}
            >
              {" "}
